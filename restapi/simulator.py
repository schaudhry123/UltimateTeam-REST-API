...
def compare_teams(team1, team2):
    ...
    for player in team1_players:
        if(player.position != 'Keeper'):
            ...
            team1_chance[player.name] = conversion/spg * random.uniform(.9, 1.3)
        else:
            team1_keep = 10.0 - random.uniform(0.7, 1.1) * float(player.rating)
            
    for player in team2_players:
        if(player.position != 'Keeper'):
            conversion = float(player.goals)/max(1, float(player.appearances))
            spg = float(player.shots_per_game)
            team2_chance[player.name] = conversion/spg * random.uniform(.9, 1.3)
        else:
            team2_keep = 10.0 - random.uniform(0.7, 1.1) * float(player.rating)
